...
class AccountHolderServiceTest {
    ...
    @Test
    void findById() {
        assertEquals(accountHolder.getBirthday(),accountHolderService.getById(admin,accountHolder.getId()).getBirthday());
    }

    @Test
    void findByIdSecurityException() {
        assertThrows(SecurityAccessException.class,() -> accountHolderService.getById(accountHolder2,accountHolder.getId()).getBirthday());
    }
