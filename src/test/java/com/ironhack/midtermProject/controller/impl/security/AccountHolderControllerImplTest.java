...
class AccountHolderControllerImplTest {
    ...
    private AccountHolderViewModel  accountHolder;
    ...
    @BeforeEach
    void setUp() {
        accountHolder = new AccountHolderViewModel(1,"user","password", "firstname",
        ...
        List<AccountHolderViewModel> list = Collections.singletonList(accountHolder);

        when(accountHolderService.findAll()).thenReturn(list);
        when(accountHolderService.getById(any(User.class), eq(accountHolder.getId()))).thenReturn(accountHolder);
